...
class place:
    ...
    def addNeighborsBlocks(self, grid):
        i = self.i
        j = self.j
...
def main():
    ...
    if len(openSet) > 0:
        ...
        if current == end:
            ...
            Tk().wm_withdraw()
            result = messagebox.askokcancel('Path founded', ('The shortest Path is ' + str(temp) + ' blocks long, \n Restart the Program?'))
            if result == True:
        ...
        for i in range(len(neighbors)):
            ...
            neighbor.h = searching(neighbor, end)
...
for i in range(cols):
    for j in range(row):
        grid[i][j].addNeighborsBlocks(grid)
